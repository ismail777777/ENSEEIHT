...
(******************************************************************************)
(*   fonction qui enlève un élément d'un arbre                                *)
(*   signature  : retrait_arbre : 'a list -> 'a arbre -> 'a arbre                  *)
(*   paramètres : - une liste d'éléments                                                *)
(*                - un arbre n-aire                                           *)
(*   résultat   : l'arbre n-aire sans l'élément à retirer                     *)
(******************************************************************************)
let rec retrait_arbre lc (Noeud (b, lb)) =
   match lc with
      ...
      | t::q -> match recherche t lb with
                | None -> Noeud(b, lb)
                | Some abr -> Noeud(b, maj t (retrait_arbre q abr) lb)


let rec parcours (Noeud (b, lb)) =
  match lb with
    |[] -> []
    |(t,a)::q -> let Noeud(b0,_)=a in
                 if b0 then [t]::(List.map(fun l -> t::l) (parcours a))@(parcours (Noeud(b, q)))
                 else (List.map(fun l -> t::l) (parcours a))@(parcours (Noeud(b, q)))
