...
struct
  ...
  let rec dechiffrer l (Noeud(lc)) = let rec aux l0 abr = match l0 with 
   |[] -> []
   |t::q -> match get_branche t abr with
            |None -> raise CodeNonValide
            |Some a -> match a with
                        |Lettre c -> c::(dechiffrer q (Noeud(lc)))
                        |_ -> aux q a
  in aux l (Noeud(lc))
  ...
  let rec arbre_to_liste a = let rec arbre2liste_aux l =
    let ajouter e l0 = List.map(fun (x,y) -> (x,e::y)) l0 in
      match l with
        |[] -> []
        |(b,a1)::q -> (ajouter b (arbre_to_liste a1))@(arbre2liste_aux q)
  in match a with
     |Lettre c -> [(c,[])]
     |Noeud(l) -> arbre2liste_aux l
 ...
 *)
  ...
  let rec chiffrer l (Noeud(lc)) = let rec aux_rechercher c l0 = 
    match l0 with
    |[]->[]
    |(tc,ti)::q1 -> if c=tc then ti
                    else aux_rechercher c q1
  in match l with
   |[] -> []
   |t::q -> (aux_rechercher t (arbre_to_liste (Noeud(lc))))@(chiffrer q (Noeud(lc)))
...
struct
  ...
  (* dechiffrer : code -> (symbole_code,symbole_donnee) arbre_chiffrement -> donnee
     ...
     Exception CodeNonValide : si le code ne peut pas être déchiffré avec l'arbre de chiffrement
   *)
  let dechiffrer c a = DRDonnee.recompose(Arbre.dechiffrer (DRCode.decompose c) a)
  ...
  (* chiffrer : donnee -> (symbole_code,symbole_donnee) arbre_chiffrement -> code
     ...
     Exception DonneeNonValide : si la donnéee ne peut pas être chiffrée avec l'arbre de chiffrement 
   *)
  let chiffrer d a = DRCode.recompose (Arbre.chiffrer (DRDonnee.decompose d) a)
