...
let rec height tr = match tr with
 |Leaf _0 -> 0
 |Node(td,tg) -> 1 + max (height td) (height tg)
...
|Leaf _0 -> 1
|Node(tg,td) -> 1 + num_nodes tg + num_nodes td
...
|Leaf c -> if c=_0 then 0
           else
            1
|Node(tg,td) -> num_values tg + num_values td
 ...
 *)
let rec bits n addr = if n=0 then []
else
    (addr mod 2)::bits (n-1) (addr/2)
 ...
 *)
let rec search tr addrl = if List.length addrl < height tr then failwith "erreur"
else
    match tr with
     |Leaf c -> c
     |Node(tg,td) -> if List.hd addrl = 0 then search tg (List.tl addrl)
                     else
                        search td (List.tl addrl)
...
let rec sprout x addrl = match addrl with
 |[] -> Leaf x
 |t::q -> if t=0 then Node(sprout x q,Leaf _0)
          else
 ...
 *   devrait pas arriver !
 *)
let rec update tr x addrl = match tr,addrl with
 |Leaf _,_ -> Leaf x
 |Node(tg,td),t::q -> if t=0 then Node(update tg x q,td)
                      else
                        Node(tg,update td x addrl)
 |_,_ -> failwith "erreur"
