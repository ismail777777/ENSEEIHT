...
struct
  ...
  let rec string_of_expression e =
    ...
    | Ident a -> (string_of_affectable a)^" "
  ...
  let rec string_of_instruction i =
    ...
    | Loop (b) -> "Loop : LOOP "^((List.fold_right (fun i tq -> (string_of_instruction i)^tq) b ""))^"\n"
    | LoopId (n,b) -> "LoopId : LOOP : "^n^((List.fold_right (fun i tq -> (string_of_instruction i)^tq) b ""))^"\n"
    | Break -> "Break "^"\n"
    | BreakId (n) -> "BreakId : Break "^n^"\n"
