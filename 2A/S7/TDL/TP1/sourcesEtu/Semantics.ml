...
and ruleIf env cond bthen belse =
  ...
  match value with
  | ErrorValue _ as result -> result
  | BooleanValue b -> if b=true then value_of_expr bthen env
                      else
                        value_of_expr belse env
  |_ -> ErrorValue UndefinedExpressionError
...
and ruleCallByName env fexpr pexpr =
  (* A traiter*)
 (* let value = value_of_expr pexpr env in
  match value with
  | ErrorValue _ as result -> result
  | _ -> fexpr value *)
...
and ruleCallByValue env fexpr pexpr =
  ...
  match value with
  | ErrorValue _ as result -> result
  | _ -> let value2 = value_of_expr fexpr env in
        match value2 with
          | ErrorValue _ as result -> result
          | FrozenValue(FunctionNode(x,retour),env_def) -> value_of_expr retour ((x,value)::env_def)
          | _ -> ErrorValue TypeMismatchError
(* ========================================================*)
